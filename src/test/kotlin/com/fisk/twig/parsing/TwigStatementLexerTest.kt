...
class TwigStatementLexerTest : TwigLexerTest() {
    ...
    fun testPlainSetNumberStatement() {
        ...
        result.shouldMatchTokenTypes(STATEMENT_OPEN, WHITE_SPACE, TAG, WHITE_SPACE, VARIABLE, WHITE_SPACE, EQUALS, WHITE_SPACE, NUMBER, WHITE_SPACE, STATEMENT_CLOSE)
        result.shouldMatchTokenContent("{%", " ", "set", " ", "foo", " ", "=", " ", "123", " ", "%}")
    }

    fun testPlainSetBooleanStatement() {
        val result = tokenize("{% set foo = true %}")
        result.shouldMatchTokenTypes(STATEMENT_OPEN, WHITE_SPACE, TAG, WHITE_SPACE, VARIABLE, WHITE_SPACE, EQUALS, WHITE_SPACE, BOOLEAN, WHITE_SPACE, STATEMENT_CLOSE)
        result.shouldMatchTokenContent("{%", " ", "set", " ", "foo", " ", "=", " ", "true", " ", "%}")
    }

    fun testPlainSetVarStatement() {
        val result = tokenize("{% set foo = bar %}")
        result.shouldMatchTokenTypes(STATEMENT_OPEN, WHITE_SPACE, TAG, WHITE_SPACE, VARIABLE, WHITE_SPACE, EQUALS, WHITE_SPACE, VARIABLE, WHITE_SPACE, STATEMENT_CLOSE)
        result.shouldMatchTokenContent("{%", " ", "set", " ", "foo", " ", "=", " ", "bar", " ", "%}")
    }

    fun testPlainSetStringStatement() {
        val result = tokenize("{% set foo = '123' %}")
        result.shouldMatchTokenTypes(STATEMENT_OPEN, WHITE_SPACE, TAG, WHITE_SPACE, VARIABLE, WHITE_SPACE, EQUALS, WHITE_SPACE, STRING, WHITE_SPACE, STATEMENT_CLOSE)
        result.shouldMatchTokenContent("{%", " ", "set", " ", "foo", " ", "=", " ", "'123'", " ", "%}")
    }
